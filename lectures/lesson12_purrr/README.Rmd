...
for (idx in seq_along(t$x)) {     # sequence
  # print(t[[idx]])  
  output[idx] <- mean(t$x[[idx]])
...
#ipsm <- vector('list', length(iphoneSales) - 1)
map(.x = select(iphoneSales, -year), .f = ~ mean(.x, na.rm = T))
...
southernStates <- c('Louisiana', 'Alabama', 'Mississippi', 'Georgia', 'South Carolina')

bad_drivers_region <- bad_drivers %>%
                        mutate(Region = case_when(state %in% southernStates ~ 'South',
