...
```{r}
monthUrl <- 'https://www.usclimatedata.com/climate/baton-rouge/louisiana/united-states/usla0033/2018/10'

session <- html_session(monthUrl) 
session %>% html_nodes('div#history') %>% 
  html_nodes('table.daily_climate_table') %>% 
  html_table() -> working
daily_climate_table <- working[[1]]
  # html_nodes('tbody')
# companyPage %>% html_node('span.cmp-header-rating-average') %>% html_text()
